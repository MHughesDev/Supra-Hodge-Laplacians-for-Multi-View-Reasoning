...
%% ── Abstract ─────────────────────────────────────────────────────────────────
\begin{abstract}
We introduce the \emph{Supra-Hodge Laplacian}, a new spectral operator that unifies two previously separate generalizations of the classical graph Laplacian: the supra-Laplacian, which encodes cross-layer coupling in multilayer graph systems, and the combinatorial Hodge Laplacian, which encodes higher-order topological structure within a single simplicial complex. Fixing a family of complexes on a shared vertex set (one view per layer), an aligned reference set of $p$-simplices, and nonnegative scalar couplings between layers, the order-$p$ Supra-Hodge Laplacian $L_p$ is the block operator on stacked ambient $p$-chains that augments embedded layerwise Hodge Laplacians $H_p^{(i)}$ with a Kronecker-type coupling $L_{\mathrm{inter}} \otimes I_{N_p}$ from the inter-layer graph. For a stacked signal, its quadratic energy splits into within-layer topological smoothness and cross-layer disagreement.

The central formal result is the Consensus-Harmonic Nullspace Theorem: $\ker(L_p)$ is exactly the set of stacked signals that are simultaneously harmonic in every layer and mutually consensual across every connected component of the inter-layer coupling graph. The construction is provably consistent: in the single-layer, order-$0$, no-higher-order limit it recovers the standard graph Laplacian; at $p=0$ it reduces to the classical supra-Laplacian; and at $k=1$ it reduces to the single-layer Hodge Laplacian. Spectral analysis yields a generalized Fiedler theory for coupled higher-order systems, with low-energy eigenmodes identifying coherent multi-view harmonic structure and conflict-frontier modes identifying topological inconsistency across views. We develop a low-energy projector $P_{p,r}$ that achieves principled spectral compression of multi-view chain signals. Proof-of-concept experiments on synthetic relational benchmarks support the theoretical predictions and show advantages over pairwise and single-view spectral baselines in this controlled setting.
\end{abstract}

\noindent\textbf{Keywords:} Hodge Laplacian, supra-Laplacian, simplicial complexes, multilayer graphs, spectral methods, higher-order topology, multi-view biological networks, scientific relational systems, spectral compression.
...
Modern relational systems are often neither pairwise nor single-view. A protein-interaction system may simultaneously encode physical binding, regulatory co-expression, metabolic dependence, and shared pathway participation over the same set of molecular entities, and some of these relations may involve higher-order groups rather than isolated pairs. A scientific knowledge network may contain citation, methodological dependence, co-authorship, and experimental-validation layers over a shared set of papers, authors, or concepts, while certain coherent groups of three or more entities carry structure that cannot be reduced to pairwise edges alone. More generally, multi-view biological and scientific systems frequently combine several relational views with genuinely higher-order interactions.
...
view. The primary contribution of this paper is mathematical; later sections only briefly discuss possible application directions in biological and scientific-network settings that exhibit coupled multi-view higher-order structure.

\subsection{Why Pairwise Graph Methods Are Insufficient}
\label{subsec:insufficient}

Consider three proteins forming a functional complex whose stability is a genuinely three-body phenomenon: remove any one interaction and the complex no longer forms, even though the pairwise interaction score between each pair remains nonzero. A graph Laplacian encodes only the pairwise structure; it cannot represent the qualitatively different status of the filled triangle versus the hollow cycle of three edges. This is not a quirk: it is the defining limitation of all pairwise methods.
...
\item \textbf{Possible application directions.} We briefly discuss several biological and scientific-network domains in which coupled higher-order multi-view structure may arise, while keeping the paper's claims mathematical and exploratory.
...
Section~\ref{sec:application} briefly discusses possible future application directions in biological and scientific-network settings.
...
\subsection{Degree Matrices and the Graph Laplacian}
...
\label{ex:4node}
Let $G$ have vertex set $V=\{1,2,3,4\}$. The undirected edges are
$\{1,2\}$, $\{2,3\}$, $\{3,4\}$, $\{4,1\}$, and $\{1,3\}$, each with unit weight. Then
\begingroup
...
\]
Each $G^{(i)} = (V, E^{(i)}, w^{(i)})$ encodes a distinct relation type; for
example, physical interaction, co-expression, pathway, or citation layers in a multi-relational biological or scientific system.
...
For any stacked signal $v=(v^{(1)},\allowbreak \dots,\allowbreak v^{(k)})$,
\begingroup
\small
\[
  \begin{aligned}
...
hollow cycle of three edges. If three proteins jointly participate in a functional module in a
way that no pair of pairwise measurements captures independently, the supra-Laplacian assigns the same energy
...
\subsection{Examples from Biology and Scientific Networks}

In a protein-complex layer of a biological network, three proteins $\{P_1, P_2, P_3\}$ may jointly form a stable functional unit even though no pair alone captures the full biological role. This is naturally represented by a 2-simplex rather than by three unrelated edges. Its presence eliminates the harmonic 1-cycle formed by the three pairwise interaction edges, indicating that the triplet is a genuine higher-order unit rather than merely a hollow loop of pairwise relations.

A similar phenomenon may arise in a scientific network. Three papers or concepts may form a tightly integrated methodological cluster whose collective relation is stronger than any pairwise association taken alone. Representing that cluster by a 2-simplex distinguishes a genuinely higher-order scientific grouping from a mere triangle in the pairwise graph.
...
\subsection{\texorpdfstring{$p$-Simplices and $p$-Chains}{p-Simplices and p-Chains}}
...
\subsection{\texorpdfstring{Recovery of the Graph Laplacian at $p=0$}{Recovery of the Graph Laplacian at p=0}}
...
\subsection{\texorpdfstring{The $p=1$ Case and Cycle-Level Inconsistency}{The p=1 Case and Cycle-Level Inconsistency}}
...
A multi-relational biological or scientific system may encode, for instance, physical interaction, co-expression, and pathway-participation relations over the same molecular entities, or citation, co-authorship, and methodological-dependence relations over the same papers and authors. Each view has its own higher-order structure (for instance, filled triangles versus hollow cycles at $p=1$). Merging all views into a single complex conflates relations that belong to different modalities and destroys the per-view interpretation of harmonic cycles. We need a family of complexes and a principled coupling.
...
$i,j$), then corresponding simplices refer to the same oriented simplex in each layer and the natural
...
correspondence, identity coupling may conflate simplices whose interpretations differ across modalities.
In this case, an estimated or structured coupling matrix $C^{(ij)}\in\R^{N_p\times
...
\subsection{\texorpdfstring{Stacked $p$-Chain Signal Spaces}{Stacked p-Chain Signal Spaces}}
...
\subsection{\texorpdfstring{Block-Matrix Definition of $L_p$}{Block-Matrix Definition of L\_p}}
...
\subsection{\texorpdfstring{Aggregated Multi-Order Operator $L_{\mathrm{SH}}$}{Aggregated Multi-Order Operator L\_SH}}
...
For any stacked $p$-chain signal $v=(v^{(1)},\allowbreak \dots,\allowbreak v^{(k)})\in\calC_p$,
\begingroup
\small
\[
  \begin{aligned}
...
\item The first nonzero eigenvalues mark the lowest-energy departures from exact consensus or harmonicity.
\end{itemize}
...
identical across layers. Such modes describe the most coherent multi-view
$p$-chain patterns under the coupled energy.
...
partial agreement meets partial inconsistency. Geometrically, they highlight
stacked signals that are smooth in some layers but inconsistent across others.
...
Real relational systems evolve: entities accrue new relations, measurements update,
and the simplicial structure in one or more views changes with a time parameter.
We write $K^{(i)}(t)$ for the time-indexed complex in layer $i$ and $L_p(t)$ for
the resulting Supra-Hodge Laplacian.
...
\subsection{Interpretation for Evolving Biological and Scientific Relational Systems}

The dynamic perspective developed above is not limited to a single static family of complexes. In many biological and scientific-network settings, the underlying relational system may evolve as simplices are added, removed, or reweighted within layers, and as the couplings between layers vary over time. In a biological context, new experimental measurements may refine interaction, co-expression, or pathway layers; in a scientific-network context, new publications, citations, or collaborative ties may alter the layered structure. The corresponding spectral quantities of the Supra-Hodge Laplacian then provide a way to monitor how coherent multi-view higher-order structure changes under perturbation. Low-energy modes may appear, disappear, or reorganize as the system evolves, while changes in the smallest positive eigenvalues may signal shifts in stability, agreement, or structural bottlenecks.


%% ══════════════════════════════════════════════════════════════════════════════
\section{Possible Application Directions}
\label{sec:application}
%% ══════════════════════════════════════════════════════════════════════════════

\subsection{Why the operator may be useful beyond pure theory}

The Supra-Hodge Laplacian is motivated by a structural pattern that arises naturally in several biological and scientific-network settings: a system may exhibit multiple relational views over a shared entity set, while each view may also contain interactions that are genuinely higher-order rather than reducible to pairwise edges. In such settings, one may wish to analyze both within-view topological structure and cross-view agreement within a single spectral formalism. The operator developed in this paper is designed precisely for that setting.

From this perspective, the present work should be read first as a mathematical construction and analysis. Its value for applications, if any, would lie in providing a principled operator for systems in which layered relational structure and higher-order topology are both essential. The role of this section is therefore not to claim validated performance on real biological or bibliometric benchmarks, but only to indicate several contexts in which such structure may plausibly arise.

\subsection{Biological network settings}

One possible family of applications consists of multi-relational biological systems. A shared set of entities such as proteins, genes, metabolites, or species may participate simultaneously in several relational views, including physical interaction, co-expression, pathway dependence, regulatory influence, or ecological co-occurrence. Within a given view, some patterns may be more naturally represented by simplices than by edges alone; for example, a higher-order biological module or jointly acting regulatory group may not be faithfully reducible to pairwise relations. In such settings, the operator may provide a way to study coherent low-energy structure, disagreement across views, and the persistence or collapse of higher-order cycles under coupling.

\subsection{Scientific network settings}

A second possible family of applications consists of scientific and scholarly networks. Here one may imagine several layers encoding citation relations, co-authorship structure, methodological dependence, shared datasets, or experimental validation over a common set of papers, researchers, or concepts. Some higher-order groupings may express joint methodological compatibility, shared evidentiary support, or tightly linked research clusters that are not adequately described by pairwise links alone. The Supra-Hodge Laplacian could, in principle, serve as a descriptive spectral summary for aligned multi-view structure in such systems.

\subsection{Scope and caution}

These application directions are intentionally hypothetical. The present paper does not establish external benchmark gains in any specific domain, nor does it claim that the operator has yet been validated on real-world biological or scientific-network datasets. The main contribution remains the mathematical construction of the operator, its reduction properties, its spectral interpretation, and its controlled proof-of-concept validation on synthetic benchmarks.
...
\subsection{Simplicial Lifting from Similarity Scores and Structural Rules}

The first computational step is constructing simplicial complexes from pairwise and triplet similarity or interaction scores on entities (for example, experimentally derived affinities or inner products of fixed feature vectors), together with closure rules that enforce the simplicial axioms.
Algorithm~\ref{alg:lifting} performs this lifting.

\begin{algorithm}[h]
\caption{Simplicial Lifting}
\label{alg:lifting}
\begin{algorithmic}[1]
\Require Feature vectors $\{f_v\}_{v\in V}$, thresholds $\tau_1,\tau_2$, dimension bound $P$
...
The complete pipeline takes a multi-view relational dataset, lifts it to a family of simplicial complexes via Algorithm~\ref{alg:lifting}, assembles $L_p$ and $L_{\mathrm{SH}}$ via Algorithm~\ref{alg:pipeline}, and returns: (i) the eigenvalue spectrum as a structural diagnostic; (ii) the Fiedler-type eigenvector $u^{(p)}_2$ for conflict or bottleneck localization; and (iii) the compressed signal $\hat{v} = P_{p,r}v$ for low-energy representation of the aligned multi-view chain signal.
...
\subsection{A Miniature Biological Two-Layer Example}

\begin{example}[Mini biological two-layer example]
\label{ex:mini_mv_rel}
Let $V = \{G, P_1, P_2, M\}$ where $G$ is a gene, $P_1$ and $P_2$ are two proteins, and $M$ is a metabolite. Consider two layers:
\begin{itemize}
\item Co-expression ($K^{(1)}$): edges $\{G,P_1\}$ and $\{G,P_2\}$.
\item Pathway-association ($K^{(2)}$): edge $\{P_1,M\}$.
\end{itemize}
Neither layer has 2-simplices. The vertex-signal $s_0 = [1,0.8,0.3,0.5]^\top$ assigns high activity or relevance to $G$ and $P_1$. The $p=0$ Supra-Hodge Laplacian $L_0$ is the supra-Laplacian of these two layers. Its Fiedler-type vector identifies the partition $\{G,P_1,M\}$ versus $\{P_2\}$: $P_2$ is present in the co-expression layer but absent from the pathway-association layer. The cross-layer disagreement energy is large because $P_2$ appears in one biological view but not the other. This example illustrates both the reduction to the classical supra-Laplacian and the multi-view diagnostic function of the framework.
\end{example}
...
\subsection{Synthetic Biological Network Case Study}

We construct a synthetic biological network with $n = 50$ entities, $k = 4$ views, and plant a topological inconsistency: in one interaction layer, three entities form a closed triangle of pairwise relations but the corresponding 2-simplex is absent, so a harmonic 1-cycle is present. The planted inconsistency manifests as a low-energy mode associated with an unfilled higher-order cycle; when the 2-simplex is added, that low-energy signature is lifted in $L_1$, while graph baselines remain unable to distinguish the two configurations.
...
HCDR degrades gracefully as Gaussian noise ($\sigma=0.1$) is added to pairwise
similarity scores used for lifting: from $0.89$ to $0.76$ for the Supra-Hodge, versus from
...
coupling materially improves both detection and compression. They do not
establish behavior on large real-world datasets where multi-view higher-order
relational structure must be constructed or estimated under noise; such
...
would allow simplex-specific coupling strengths, encoding biologically or scientifically interpretable correspondences. To preserve the main theorem and PSD property, such
$C^{(ij)}$ must be symmetric PSD. Learned structured couplings (via estimation from paired cross-layer correspondences) are a natural extension.

\subsection{Thresholded Versus Learned Lifting}

The lifting algorithm uses fixed thresholds $\tau_1,\tau_2$. Learned thresholds
(optimized against a biological or bibliometric fitting objective via a differentiable relaxation of the
thresholding step) would enable data-adaptive structural extraction. This
...
The experiments in this paper use synthetic data with planted structure and are intended only as controlled proof-of-concept validation. They demonstrate that the operator behaves in accordance with the theoretical predictions in settings where the relevant multi-view higher-order signal is explicitly constructed. They do not, however, establish performance on real-world datasets, where the appropriate simplicial structure, cross-view alignment, and evaluation targets may be substantially more ambiguous. Meaningful domain-specific validation would require benchmark settings in biological or scientific-network analysis where multi-view higher-order relational structure can be operationalized in a principled way. Such empirical evaluation is left for future work.
...
improve numerical conditioning. A fully differentiable variant along the lines of operator constructions surveyed for random walks and normalized Hodge Laplacians on simplicial complexes~\cite{schaub2020} could serve as a structure-aware regularizer when fitting statistical models to multi-relational scientific measurements.
...
Large-scale protein-interaction, gene-regulatory, and citation or co-authorship networks would test the framework at operational scale.
...
prior frameworks. The primary contribution is mathematical: the operator, its
reductions, the spectral consequences above, and the controlled synthetic validation.
...
\subsection{Possible Biological and Scientific-Network Directions}

The operator introduced here may be relevant in domains where several relational views coexist over a shared entity set and where at least some within-view structure is genuinely higher-order. Examples may include protein-interaction systems, gene-regulatory and pathway networks, ecological co-occurrence networks, citation and co-authorship networks, and experimental-dependency structures in science. At present, these should be understood only as prospective directions. The paper does not claim established application-level gains in any of these domains; rather, it provides a mathematical framework that may support such investigations in future work.
...
relevance whenever multiple higher-order relational views coexist over a shared entity set drawn from molecular biology or from scientific metadata (publications, authors, experiments, instruments). Within the coupled simplicial setting
...
\subsection{\texorpdfstring{Full Two-Layer Supra-Hodge $p=1$ Computation}{Full Two-Layer Supra-Hodge p=1 Computation}}
...
Gaussian feature vectors $f_v\sim\mathcal{N}(0,I_{32})$, normalized to unit sphere.
Similarity: $\simop(u,v)=f_u^\top f_v$. Triplet score:
